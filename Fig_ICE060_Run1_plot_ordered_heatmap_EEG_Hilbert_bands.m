%% Fig_ICE060_Run1_plot_ordered_heatmap_EEG_Hilbert_bands.m
clear; clc;
...
grpOrdered = grp(ord);   % group id in the final plotting order
...
for b = 1:numel(bandOrder)
    ...
    plot_heatmap_r_noDiag_withNames_andGroupLines(R_ord, names_ord, grpOrdered, grpLabels, ttl, outPng);
...
function [ord, grpOriginal, grpLabels] = order_ICE060_seednames(names)
% ICE060 anatomy-first ordering with laterality preserved:
%   Insula (AIN/PIN) -> Left MTL (A, AH, PH) -> Right MTL (A, AH, PH)
% Works for names like: DRAIN1-DRAIN2, DRPIN1-DRPIN2, DLAH1-DLAH2, DLPH5-DLPH6, DRAH6-DRAH7, ...
...
% ---- define block order: Left first then Right ----
...
% ---- group IDs contiguous in ORDERED list ----
anat_ord = anat(ord);
[uniqGroups,~,grp_ord] = unique(anat_ord, 'stable');  % 1..G in order

% convert grp_ord back to ORIGINAL indexing vector
grpOriginal = nan(n,1);
grpOriginal(ord) = grp_ord;
